...
def pivot_months_pandas(data):
    ...
    """
    #Add a column 'month' that contains the results of applying the date_to_month
    #function to the existing 'date' column.
    data['month'] = data['date'].apply(date_to_month)
    #print(data.head())
    
     #Use the Pandas groupby method to aggregate over the name and month columns.
    grouped_data = data.groupby(['name', 'month'])
    #print(grouped_data.head())
    
    #why did station and date column get removed from dataset!!!!!!!!!!!!!!!!
    
    #Sum each of the aggregated values to get the total.
    new_data = grouped_data.aggregate('sum').reset_index()
    #print(new_data.head())
    
    #Use the Pandas pivot method to create a row for each station (name)
    # and column for each month.
    monthly = new_data.pivot(index='name', columns='month', values='precipitation')
    #print(monthly)
    
    count_data = grouped_data.aggregate('count').reset_index()
    #print(count_data)
