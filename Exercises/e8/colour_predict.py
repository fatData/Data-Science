...
def main():
    ...
    bayes_rgb_model = GaussianNB()                                  #create naive bayes model
    
    bayes_lab_model = make_pipeline(                                 #create new naive bayes model using LAB colors
        FunctionTransformer(rgbToLabColor),
        GaussianNB()
    )
    
    
    knn_rgb_model = KNeighborsClassifier(n_neighbors=7)             #create KNN model
    
    knn_lab_model = make_pipeline(                                  #create KNN model using LAB colors
        FunctionTransformer(rgbToLabColor),
        KNeighborsClassifier(n_neighbors=7)
    )
    
    
    svc_rgb_model = SVC(kernel='linear', C=8)
    ...
    for i, m in enumerate(models):                                                                              #take each model, train it and save plotted predictions 
